...
int main(int argc, char *argv[])
{
    ...
    char beammap_path[STR_SIZE];//name of pre-existing beammap file
    char obs_filename[STR_SIZE];//filename to be extracted from obs_filepath
    ...
    strcpy(beammap_path,argv[2]);
    printf("beammap_file_path: %s\n",beammap_path);
...
int connect_to_roach(int roachno)
{
    ...
    req.tv_sec = 0;
    req.tv_nsec = usec * 1000L;
    char* hostnames[] = {"10.0.0.14","10.0.0.15","10.0.0.16","10.0.0.17"};//,"10.0.0.11","10.0.0.12","10.0.0.13","10.0.0.14","10.0.0.15","10.0.0.16","10.0.0.17"};
...
void copy_beam_file_tree(char* obs_filepath,char* beammap_path,char beam_data[BEAM_ROWS][BEAM_COLS][STR_SIZE])
{
    ...
    beammap_file = H5Fopen(beammap_path,H5F_ACC_RDWR, H5P_DEFAULT);
    if (beammap_file < 0)
    {
        fprintf(stderr,"Error opening beammap file %s : ",beammap_path);
        error("");
    }
    ...
    if (bool_write_quicklook == 1)
    {
        for (row=0; row < BEAM_ROWS;++row)
        {
            for (col = 0; col < BEAM_COLS;++col)
            {
                ...
                if (channel < NROACHES*NPIXELS_PER_ROACH)
                {
                    quicklook_image[row][col] = photon_counts[current_sec][channel];
                }
                else
                {
                    quicklook_image[row][col] = 0;
                }
            }
        }
