...
//when set =1, a whole array of resonators is tested, otherwise, only a few resonators exist
#define FULLARRAY 0
...
int main(int argc, char *argv[])
{
	...
	printf("Ready to receive data from roaches\n");
		...
		for (r=0;r<NROACHES;++r)
		{
			if (sec[r] < exptime)
			{
				...
				if (N_bytes >= BUFSIZE)
				{
					...
					printf("Found a roach with buffer ready\n");
					fflush(stdout);
				}
			}
		}
...
//				}
				...
				if (sec[ready_roach] < exptime)
				{
					if (packet == (uint64_t)(-1))
					{	
						printf("second %d of %d for roach %d at %.3f \n",sec[ready_roach],exptime,ready_roach,current_time());
						if (errno != 0)
...
int connect_to_roach(int roachno)
{
    ...
    req.tv_sec = 0;
    req.tv_nsec = usec * 1000L;
    char* hostnames[] = {"10.0.0.10","10.0.0.13","10.0.0.10","10.0.0.11"};
