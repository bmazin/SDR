...
FreqTable::FreqTable(unsigned int seed)
	:required_min_neighbor_separation(NULL),required_max_neighbor_separation(NULL),collision_threshold(0.5),scatter_stddev(0.5),gradient_magnitude(0),gradient_angle(0),num_bad_pixels(0),min_neighbor_separation(0),MAX_FREQ(512),NROWS(46),NCOLS(6),num_attempts_to_solve(0),solution_found(false),has_if_hole(false),IF_freq_hole_width(0.)
{
	clean_table = NULL;
	real_table = NULL;
...
void FreqTable::set_basic_requirements(int num_rows,int num_cols, double bandwidth, double start_f) 
{
	NROWS = num_rows;
	NCOLS = num_cols;
...
void FreqTable::set_if_hole(bool if_hole, double if_hole_width)
//if hole will be a hole in the frequencies at the center of the tile's bandwidth
{
    has_if_hole = if_hole;
	IF_freq_hole_width = if_hole_width;
}
...
bool FreqTable::check_min_neighbor_separation(int r, int c, double center_value,bool check_row_below)
{
	...
	for (int i = 0; i < N_neighbors; ++i)
	{
		...
		if (r+dr >= 0 && r+dr < NROWS && c+dc >=0 && c+dc < NCOLS)
		{
            ...
            if (dist < required_min_neighbor_separation)
                return false;
		}
	}
    return true;
...
{
    if (required_max_neighbor_separation == NULL)
    {
        return true;
    }
	double max_dist = 0;
	...
	for (int i = 0; i < N_neighbors; ++i)
	{
		...
		if (r+dr >= 0 && r+dr < NROWS && c+dc >=0 && c+dc < NCOLS)
		{
            ...
            if (r == 16 || r == 17)
                cout << r+dr << " " << c+dc << " " << real_table[r+dr][c+dc] << " " << dist << endl;
                */
            if (dist > required_max_neighbor_separation)
            {
                /*
                if (r == 16 || r == 17)
                    cout << dist << ">" << required_max_neighbor_separation << endl;
                    */
                return false;
            }
		}
...
void FreqTable::find_layout_solution(bool work_backward,bool seedCornerWithLowest)
{
	initialize_clean_table(work_backward,seedCornerWithLowest);
}

void FreqTable::initialize_clean_table(bool work_backward, bool seedCornerWithLowest)
{
	build_freq_list();
	...
	while (solved == false)
	{
		...
		if (N_table_attempts % 10000 == 0)
		{
			...
			cout << "furthest row " << furthest_row << endl;
            fflush(stdout);
		}
		++N_table_attempts;
		int random_index = 0;
        ...
        for (int r = r_start; (!work_backward && r <= r_end) || (work_backward && r>= r_end); r+=r_increment)
		{
            for (int c = c_start; (!work_backward && c <= c_end) || (work_backward && c>= c_end); c+=c_increment)
			{
                ...
                if (seedCornerWithLowest == true && c == c_start && r == r_start)
                {
                    freq = available_freq_list[0];
                    clean_table[r][c] = freq;
                    real_table[r][c] = freq;
                    available_freq_list.erase(available_freq_list.begin());
                }
                else
                {
				...
				if (give_up == true)
				{
                    if (r > furthest_row && work_backward == false)
                        furthest_row = r;
                    else if (r < furthest_row && work_backward == true)
                        furthest_row = r;
                    give_up_row = r;
                    give_up_col = c;
					break;
