...
class AppForm(QMainWindow):
    ...
    def snapAndPulses(self):
        ...
        addr_start = self.roach.read_int('pulses_addr')
        addr0=addr_start
        print 'addr_start: ',addr_start
        for n in range(steps):
            self.roach.write_int('startSnap', 0)
            self.roach.write_int('snapPhase_ctrl', 1)
            self.roach.write_int('snapPhase_ctrl', 0)
            ...
            print addr1,
            time.sleep(0.001)
            addr2 = self.roach.read_int('pulses_addr')
            print ' ',addr2
            bin_data_phase = bin_data_phase + self.roach.read('snapPhase_bram', 4*L)    
        addr_last = self.roach.read_int('pulses_addr')
        addr1=addr_last
    ...
    def snapshot(self):        
        ...
        numpy.savetxt('phase_timestream',phase)
        self.canvas.draw()
