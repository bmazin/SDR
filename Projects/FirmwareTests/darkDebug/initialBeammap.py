...
def xyPack(feedline=1,boardRange='a',sideband='pos',freqChannel=0):
    ...
    yChannelOffset = freqChannel // nCols #for each filled row (containing nCols), wrap to the next row
    y = yChannelOffset + ySidebandOffset + yFeedlineOffset
    x = freqChannel % nCols #x position is remainder after filling rows
    return {'x':x,'y':y,'row':y,'col':x,'feedline':feedline,'sideband':sideband,'boardRange':boardRange,'freqChannel':freqChannel}
...
def xyUnpack(**kwargs):
    ...
    freqChannel = (yChannelOffset * nCols) + x
    return {'x':x,'y':y,'row':y,'col':x,'feedline':feedline,'sideband':sideband,'boardRange':boardRange,'freqChannel':freqChannel}
