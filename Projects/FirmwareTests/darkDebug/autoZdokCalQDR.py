...
def checkRamp(rampVals,nBits=12,bitsRemoved=0, bPlot=False):
    foundGoodStart = False
    #bitsRemoved = 2
    nBits = nBits-bitsRemoved
    scaleFactor = (2.**bitsRemoved) 
    maxRampVal = 2**(nBits)
    offset = 2**(nBits-1)
    deriv = np.diff(rampVals)
    rampStep = 1
    errorsFound = []
    for i in range(2**bitsRemoved):
        rampModel = np.floor((np.arange(0,len(rampVals),rampStep)+i)/scaleFactor)
        rampModel = rampModel + rampVals[0] + offset
        rampModel = (np.array(rampModel,dtype=np.int) % maxRampVal)-offset
        rampErrors = rampVals-rampModel
        errorsFound.append(np.any(rampErrors != 0))
        if bPlot:
    ...
    #rampModel = offset+rampVals[0]*4+np.arange(0,len(rampVals),rampStep)
    #rampModel = (np.array(rampModel,dtype=np.int) % maxRampVal)-offset
    #rampModel = np.floor(rampModel/4.)
    #rampErrors = rampVals - rampModel
    #errorsFound = np.any(rampErrors != 0)

    

    return np.all(errorsFound)
...
def findCal(fpga,bitsRemoved=0,bPlot=False):
    snapDict = snapZdok(fpga)
    errorInI = checkRamp(snapDict['iVals'],bitsRemoved=bitsRemoved,bPlot=bPlot)
    errorInQ = checkRamp(snapDict['qVals'],bitsRemoved=bitsRemoved,bPlot=bPlot)
    ...
    if initialError == False:
        ...
        nSteps = 60
        stepSize = 4
        failPattern = findCalPattern(fpga,bitsRemoved,nSteps=nSteps,stepSize=stepSize)['failPattern']
...
def findCalPattern(fpga,bitsRemoved,bPlot=False,nSteps=60,stepSize=4):
