...
def generateComb(freqs,nSamples=2**23,amplitudes=None,phases=None,sampleRate=2e9,nBitsPerSampleComponent=12,dynamicRange=1.,phasePulseDicts=None):
    toneDict = generateTones(freqs=freqs,nSamples=nSamples,amplitudes=amplitudes,phases=phases,sampleRate=sampleRate,nBitsPerSampleComponent=nBitsPerSampleComponent,dynamicRange=dynamicRange,phasePulseDicts=phasePulseDicts)
...
def generateInterleavedTones(freqs,nSamples=8192,amplitudes=None,phases=None,sampleRate=2e6,nBitsPerSampleComponent=16,dynamicRange=1.,nSamplesPerCycle=2,phasePulseDicts=None):
...
def generateTones(freqs,nSamples=2**23,amplitudes=None,phases=None,sampleRate=2e9,nBitsPerSampleComponent=12,dynamicRange=1.,phasePulseDicts=None):
    #,pulseAmpDeg=None,pulseDecayTime=30.e-6,pulseRiseTime=.5e-6,pulseArrivalTime=50.e-6):
...
#    out = toneAmp*np.exp(1.j*(2*np.pi*freq*time+phase+initialPhase))
    ...
    for iFreq,freq in enumerate(quantFreqs):
        if not (phasePulseDicts is None):
            ...
            #create a decaying exponential for the tail
            pulse = np.exp(-t[0:nSamples-pulseArrivalIndex]/decayTime)
            #create an exponential and reverse it for the inital rise of the pulse
            #don't include the peak in the rise since it is included in the fall
            rise = np.exp(-t[1:pulseArrivalIndex+1]/riseTime)[::-1]
            paddedPulse = np.append(rise,pulse)
            phasePulse = pulseAmp*paddedPulse
        else:
            phasePulse = 0.
            
        phi = 2.*np.pi*freq*t
        expValues = np.exp(1.j*(phi+phases[iFreq]+phasePulse))
        iValues = np.real(expValues)
        qValues = np.imag(expValues)
...
def loadWaveToMem(fpga,waveFreqs=[10.4e6],phases=None,sampleRate=2.e9,nSamplesPerCycle=8,nSamples=2**23,nBytesPerMemSample=8,nBitsPerSamplePair=24,memNames = ['qdr0_memory','qdr1_memory','qdr2_memory'],memType='qdr',dynamicRange=1.,phasePulseDicts=None):
    ...
    tone = generateComb(waveFreqs,phases=phases,nSamples=nSamples,sampleRate=sampleRate,dynamicRange=dynamicRange,phasePulseDicts=phasePulseDicts)
