...
; find chisq       
    for i=0L,pcount-1L do begin
        ...
        ;if( max(P1[980:1020]*180.0/!Pi) GT 20.0 AND max(P2[980:1020]*180.0/!Pi) GT 20.0) then begin
          ; align on maximum using optimal filter time offset
           ...
           chisq[i] = total( abs(P1f-ch1b[i]*FFT(shift(t1[900:1699],mx-20),-1)*!Pi/180.0)^2/ch1nc)*(1.00d-6*800.0)/800.0
           ...
           ;if( chisq[i] GT 0.6 AND chisq[i] LT 0.7 ) then begin
   ...
   ; set chisq limits
   chisqhist = histogram( chisq, MIN = 0, MAX = 10, BINSIZE = 0.01)
   bins = FINDGEN(N_ELEMENTS(chisqhist))*0.01
   chisqcent = bins[(where( chisqhist EQ max(chisqhist)))[0]]
   ...
   ; make template
       for i=0L,pcount-1L do begin
        ...
        ;if( max(P1[980:1020]*180.0/!Pi) GT 20.0 AND max(P2[980:1020]*180.0/!Pi) GT 20.0) then begin
          ; align on maximum using optimal filter time offset
           ...
           if( chisq[i] GT chisqcent-0.1 AND chisq[i] LT chisqcent+0.1 ) then begin
            t1a[20:1980] += P1[ks-980:ks+980]/max(v1)
            n1c += 1.0
           endif
    ...
    ; output templates and noise spectra
    openw,2,path+roach_str+pixel_str+'Template-3pass.dat'
    for i=0,1999 do begin
      printf,2,idx[i]*1.00,t1a[i]
    endfor
    close,2
    
    openw,3,path+roach_str+pixel_str+'TemplateFit-3pass.dat'
    for i=0,1999 do begin
