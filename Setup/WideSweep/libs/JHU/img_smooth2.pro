...
;       IMG_SMOOTH2
; PURPOSE:
;       Multi-pass box smooth a 2-D or 3-D image array.
; CATEGORY:
; CALLING SEQUENCE:
;       out = img_smooth2(in,width)
; INPUTS:
;       in = Input image.                    in
...
;         Uses smooth2, multi-box smoothing.
...
;-
;-------------------------------------------------------------
	function img_smooth2, img, width, error=err, help=hlp
 
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  print,' Multi-pass box smooth a 2-D or 3-D image array.'
	  print,' out = img_smooth2(in,width)'
	  ...
	  print,'   Uses smooth2, multi-box smoothing.'
	...
	if tr eq 0 then begin
	  return, fix(type=dtyp,smooth2(img,width))
	endif
 
	;--------  3-D image  --------------------------
	img_split, img, r, g, b
	r = fix(type=dtyp,smooth2(r,width))
	g = fix(type=dtyp,smooth2(g,width))
	b = fix(type=dtyp,smooth2(b,width))
