...
;-------------------------------------------------------------
	...
	xdv = tmp(0)<xxdv(1)  & ydv = tmp(1)<yydv(1)
        ...
        if not keyword_set(xmode) then t=tvrd(xdv,0,1,!d.y_size,true=tvtr)
	...
	xcl = -2  & ycl = -2
	  ...
	  xcl = xdv  & ycl = ydv
	    ...
	    if vflag then tvlct,ii0,ii0,ii0	  ; Need B&W table.
            tv, t, xl, 0, true=tvtr               ; Replace last column. 
	    if vflag then tvlct,rr0,gg0,bb0	  ; Original table.
            t = tvrd(xdv,0,1,!d.y_size,true=tvtr) ; Read new column.
          endif else begin
