...
;-
;-------------------------------------------------------------
	pro relfill, xarr, yarr, ang=ang, scale=sc, center=cen, $
	  device=dev, normal=norm, data=data, color=clr, linestyle=sty, $
	  ocolor=oclr, thickness=thk, noclip=noclip, help=hlp
 
	if (n_params(0) lt 2) or keyword_set(hlp) then begin
	  print,' Relative plot with rotation, scaling, translation.'
	  print,' relfill, xarr, yarr'
	  ...
	  print,'   OCOLOR=oclr Outline color (def=no outline).'
	  print,'   LINESTYLE=sty Outline linestyle (def=!p.linestyle).'
	...
	;  Fill
	;-------------------------------------------------------
	polyfill, /dev, x, y, noclip=noclip, color=clr
 
	;-------------------------------------------------------
	;  Outline
	;-------------------------------------------------------
	if n_elements(oclr) ne 0 then begin
	  plots, /dev, x, y, noclip=noclip, color=oclr,linestyle=sty,thick=thk
	endif
