...
;-------------------------------------------------------------
	...
	pro hor, y, help=hlp, device=device, linestyle=ls, color=clr, $
	  ...
	  left=left, right=right, out=out, shrink=shrink, xrange=xran
	...
	if (n_params(0) lt 1) or keyword_set(hlp) then begin
	  ...
	  print,'   XRANGE=xran Optional X range to use (def=all).'
	  print,'     Give xran=[xmin,xmax] in specified coordinate system.'
	...
	if (lflag+rflag) eq 0 then begin
	  ...
	  rflag=1
        ...
        if keyword_set(device) then begin
          xx = [0,!d.x_size-1]
	  if n_elements(xran) eq 2 then xx=xran
	...
	;----------  Data  -------------
	  ...
	  for i = 0L, n-1 do begin
	    ...
	    if !x.type eq 0 then begin
	        ...
	        oplot,[min(xx),max(xx)]>xr(0)<xr(1),[1.,1.]*yy(i), $
