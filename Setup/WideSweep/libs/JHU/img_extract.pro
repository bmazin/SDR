...
; CATEGORY:
; CALLING SEQUENCE:
;       subimg = img_extract(img, ix, iy, dx, dy).
; INPUTS:
;       img = Input image.                            in
...
;-
;-------------------------------------------------------------
	function img_extract, img, ix, iy, dx, dy, $
	...
	if (n_params(0) lt 5) or keyword_set(hlp) then begin
	  ...
	  print,' subimg = img_extract(img, ix, iy, dx, dy).'
	...
	if (ndim lt 2) or (ndim gt 3) then begin
	  err = 1
	  print,' Error in img_extract: given array must 2-D or 3-D.'
	  return,img
	endif
	...
	if typ eq 0 then begin
	  err = 2
	  print,' Error in img_extract: given array must have a dimension of 3.'
	  return,img
	endif
