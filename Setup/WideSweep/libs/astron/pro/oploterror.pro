...
; Only draw error bars for X values within XCRANGE
 ...
 FOR i = istart, iend, Nskip DO BEGIN
    ...
    cgplots, [xx[i],xx[i]], [ylo[i],yhi[i]], LINESTYLE=est,THICK=eth,  $
           NOCLIP = noclip, COLOR = ecol,ADDCMD=window

    ; Plot X-error bars 
    ;
    if np EQ 4 then $
       cgplots, [xlo[i],xhi[i]],[yy[i],yy[i]],LINESTYLE=est, $
              THICK=eth, COLOR = ecol, NOCLIP = noclip,ADDCMD=window
    ...
    IF (hat NE 0) THEN BEGIN
       ...
       exx2 = exx1 + hln
       if lobar then $
          cgplots, [exx1,exx2], [data_low[1,i],data_low[1,i]],COLOR=ecol, $
                 LINESTYLE=est,THICK=eth,/DEVICE, noclip = noclip, $
		 ADDCMD = window
       if hibar then $
          cgplots, [exx1,exx2], [data_hi[1,i],data_hi[1,i]], COLOR = ecol,$
                 LINESTYLE=est,THICK=eth,/DEVICE, noclip = noclip, $
		 ADDCMD = window
;                                          
       IF np EQ 4 THEN BEGIN
