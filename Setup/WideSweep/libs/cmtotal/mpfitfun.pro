...
;  that the fit will still halt if the model function produces
...
;
;  $Id: mpfitfun.pro,v 1.13 2007/12/15 14:28:27 craigm Exp $
;-
; Copyright (C) 1997-2002, 2003, 2006, 2007, Craig Markwardt
...
function mpfitfun_eval, p, dp, _EXTRA=extra
  ...
  if n_elements(wts) GT 0 then begin
      ...
      if n_elements(dp0) GT 0 AND n_elements(dp) EQ np*nf then begin
          for j = 0, np-1 do dp[j*nf] = dp[j*nf:j*nf+nf-1] * wts
      endif
  endif else if n_elements(err) GT 0 then begin
      result = (y-f)/err
      if n_elements(dp0) GT 0 AND n_elements(dp) EQ np*nf then begin
          for j = 0, np-1 do dp[j*nf] = dp[j*nf:j*nf+nf-1] / err
      endif
  endif else begin
      result = (y-f)
  endelse
